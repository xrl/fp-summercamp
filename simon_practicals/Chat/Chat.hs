...
instance Eq Client where
  Client{clientName=clnm1} == Client{clientName=clnm2} = clnm1 == clnm2
instance Show Client where
  show Client{clientName=clnm} = clnm
-- >>
...
names :: Server -> Client -> STM ()
names Server{..} to = do
  clientmap <- readTVar clients
  sendMessage to $ Names (show $ Map.keys $ Map.filter (\v-> (v /= to)) clientmap)
...
-- <<readName
    readName = do
      hPutStrLn handle "What is your name? (you got 10 seconds...)"

      name_from_timeout <- timeout 10000000 (hGetLine handle)
      case name_from_timeout of
        Nothing -> do
          hPutStrLn handle "Dude, make it quicker next time."
...
handleMessage server client@Client{..} message =
  case message of
     ...
     Names payload      -> output $ "||" ++ payload
     Whoami             -> output $ "YOU ARE " ++ clientName
     Command msg ->
       case words msg of
           ["/kick", who] -> do
           ...
           ["/quit"] ->
               return False
           ["/names"] -> do
               atomically $ names server client
               return True
           ["/whoami"] -> do
               atomically $ whoami client
               return True
           ('/':_):_ -> do
